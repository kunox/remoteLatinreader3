window.onload = function () {
    ...
    $("#wbtn").on("click", postjson2);              // #wbtnは画面右にある「送信」ボタン
    
    if(!$("#saveoption").length){
        $("#header").append(spacebutton);   //buttonがすでにある可能性もある
    }
    $("#header").after(wboxhtml);       // headerのあとに追加しておく

    // $("#header").on("click",".rbtn", uploadsave)
    $("#saveoption").on("click", uploadsave);
    $("#downloadsave").on("click", downloadsave)
...
<p id="wboxtext" contentEditable="true" spellcheck="false">abc</p>
</div>
...
function xeditNote3() {
    ...
    $("#disp").on("click", ".ditems",function(){
        ...
        $("#wboxtext").html("<b>"+wx1 +"</b>," + wx2 +"<br/>" + "<b>" + wx3 + "</b><br/>" + wx4);  //***************** */
    })
}

// Saveが押されたとき
function wordsave() {
    var tword = $("#wbox").dialog("option","title");
    if(tword.length < 1){
        $("#wbox").dialog("close");
    }
    var meantext = $("#wboxtext").html();
    ...
    $.post("/postjson2", testdata, null, "json")
        .done(function (data) {
            ...
            for (var i in datajson) {
                var result = tempdiv.replace("%s1", datajson[i].Dic.WORD);
